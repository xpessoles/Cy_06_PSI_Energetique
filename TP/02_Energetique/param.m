...
%--------------------------------------------------------------    
% Données inertielles propres à l'ensemble du mécanisme
%--------------------------------------------------------------  

    meq = 3.2; %Masse équivalente de tout l'équipage mobile ramenée sur le chariot (kg): origine expérimentale pour l'instant
    Jeq = meq*(R/i)^2; % Moment d'inertie équivalent de tout l'équipage mobile ramenée sur le moteur (kg.m2)
...
I1=0.037e-3+0.012e-3+8e-8; %Moteur+géné tachy + encodeur
Ir=0.135e-4; % Réducteur ramené sur l'entrée
I2roulement=2*(8349e-9+18*7/20*0.82e-3*19.5e-3^2); %2 bagues int de roulement à billes + billes montées sur  poulie crantée
I3=4.7e-5+0.09e-4+0.013e-4+0.15e-4+I2roulement; % Poulie+accouplement (2 parties) + bagues int de roulement à billes
m4=0.16; % Courroie
m5=0.9;% Chariot : ici il faudrait rajouter les petites bricoles fixées sur le chariot.
I6=4.7e-5+0.01e-4+I2roulement; %Poulie crantée+bague int de roulement à billes
Jeq = I1+Ir+1/i^2*(I3+I6+(m4+m5)*R^2); % Ramené sur le moteur
meq=Jeq*i^2/R^2; % Ramené sur le chariot
